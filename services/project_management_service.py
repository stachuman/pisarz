...
class ProjectManagementService:
    ...
    def handle_project_selection(self, project_path: str, project_name: str) -> bool:
        """
        ...
        """
        try:
            self.logger.info(f"Loading project: {project_name} from {project_path}")
            
            # Load project using project controller
            success = self.main_window.project_controller.load_project(project_path, project_name)
            if not success:
            ...
            self.main_window.llm_controller.update_project_context(project_name, project_path)
            
            # Set up narrative context panel
            self._setup_narrative_context_panel(project_path)
    ...
    def handle_scene_selection(self, scene_id: int, scene_title: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error handling scene selection: {e}")
            return False
    ...
    def handle_scene_selection_with_search(self, scene_id: int, scene_title: str, search_query: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error handling scene selection with search: {e}")
            return False
    ...
    def _setup_narrative_context_panel(self, project_path: str):
        """Setup narrative context panel for the project."""
        try:
            if hasattr(self.main_window, 'narrative_context_panel'):
                from pathlib import Path
                self.main_window.narrative_context_panel.set_project(Path(project_path))
        except Exception as e:
    ...
    def _get_scene_content(self, scene_id: int) -> str:
        ...
        try:
            ...
            self.logger.error(f"Error getting scene content: {e}")
            return ''
